...
class Obstacle:
    def __init__(self, row: int, column: int, rows_size: int=1, columns_size: int=1, uid: Any=None):
        self.row = row
        self.column = column
    ...
    def has_collision(self, obj_corner_row: int, obj_corner_column: int, obj_size_rows: int=1, obj_size_columns: int=1):
...
async def show_obstacles(canvas: curses.window, obstacles: List[Obstacle]):
...
def _is_point_inside(corner_row: int, corner_column: int, size_rows: int,
                     size_columns: int, point_row: int, point_row_column: int) -> bool:
...
def has_collision(obstacle_corner: Sequence[int], obstacle_size: Sequence[int],
                  obj_corner: Sequence[int], obj_size: Sequence[int] = (1, 1)):
    '''Determine if collision has occured. Return True or False.'''
